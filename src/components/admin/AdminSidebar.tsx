import { LayoutDashboard, UserCog, Building2, Briefcase, ShoppingBag, MessageSquare, User, Bell, LogOut, Settings, ChevronDown, RefreshCw, FileText } from "lucide-react";
...
const navigationItems = [{
  ...
  value: "companies",
  icon: Building2,
  showKey: 'show_companies'
}, {
  title: "Oportunidades",
  value: "opportunities",
  icon: Briefcase,
  showKey: 'show_opportunities'
}, {
  title: "Marketplace",
  value: "marketplace",
  icon: ShoppingBag,
...
export function AdminSidebar({
  ...
  return <Sidebar className={cn(collapsed ? "w-14" : "w-64", "bg-background border-r")} collapsible="icon">
      {/* Header */}
      <SidebarHeader className="border-b p-4 bg-background">
        <div className="flex items-center gap-2 mb-3">
          ...
          {!collapsed && <Button variant="ghost" size="sm" className="p-1 h-auto">
              <RefreshCw className="h-3 w-3" />
            </Button>}
        </div>
        
        {!collapsed && <div className="bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 rounded-lg p-3 space-y-2">
        ...
        {!collapsed && <p className="text-xs text-muted-foreground mt-3 font-medium">Panel Administrativo</p>}
      </SidebarHeader>
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu className="space-y-2 px-2">
              {navigationItems.filter(item => customization?.[item.showKey as keyof typeof customization] !== false).map(item => {
              const isActive = activeTab === item.value;
              return <SidebarMenuItem key={item.value}>
                      <SidebarMenuButton onClick={() => onTabChange(item.value)} isActive={isActive} className={cn("cursor-pointer px-3 py-2.5 rounded-md transition-colors w-full", isActive ? "bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/50 hover:text-purple-700 dark:hover:text-purple-300" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800")}>
                        <item.icon className="h-4 w-4 flex-shrink-0" />
                        {!collapsed && <span className="truncate">{item.title}</span>}
                        {!collapsed && item.value === 'publishing-requests' && pendingCount > 0 && <Badge variant="destructive" className="ml-auto text-xs">
                            {pendingCount}
                          </Badge>}
                      </SidebarMenuButton>
                    </SidebarMenuItem>;
      ...
      <SidebarFooter className="border-t p-4 bg-background">
        ...
        <div className="space-y-2 mb-4">
          {customization?.show_chat !== false && <SidebarMenuButton onClick={() => onTabChange("chat")} isActive={activeTab === "chat"} className={cn("cursor-pointer w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-colors", activeTab === "chat" ? "bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/50 hover:text-purple-700 dark:hover:text-purple-300" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800")}>
              ...
              {!collapsed && <>
                  ...
                  {unreadMessagesCount > 0 && <Badge variant="destructive" className="text-xs flex-shrink-0">
                      {unreadMessagesCount > 9 ? "9+" : unreadMessagesCount}
                    </Badge>}
                </>}
            </SidebarMenuButton>}

          {customization?.show_notifications !== false && <SidebarMenuButton onClick={() => onTabChange("notifications")} isActive={activeTab === "notifications"} className={cn("cursor-pointer w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-colors", activeTab === "notifications" ? "bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/50 hover:text-purple-700 dark:hover:text-purple-300" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800")}>
              <Bell className="h-4 w-4 flex-shrink-0" />
              {!collapsed && <>
                  <span className="flex-1 truncate">Notificaciones</span>
                  {unreadNotificationsCount > 0 && <Badge variant="destructive" className="text-xs flex-shrink-0">
        ...
        {collapsed && <div className="flex flex-col items-center gap-2">
            <Button variant="ghost" size="sm" className="p-2 hover:bg-slate-50 dark:hover:bg-slate-800" onClick={() => onTabChange("admin-profile")}>
              <User className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" className="p-2 text-red-600 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/50" onClick={handleLogout}>
